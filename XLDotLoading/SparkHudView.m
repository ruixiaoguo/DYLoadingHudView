...
-(void)buildUI
{
    ...
    NSArray *dotBackGroundColors = @[[self R:141 G:165 B:255 A:1],[self R:224 G:151 B:255 A:1]];
...
+(SparkHudView *)getLoadingInView:(UIView *)view {
    
    SparkHudView *loading = nil;
    for (SparkHudView *subview in view.subviews) {
        if ([subview isKindOfClass:[SparkHudView class]]) {
            loading = subview;
        }
    }
    return loading;
...
+(void)show
{
    SparkHudView *loading = [SparkHudView getLoadingInView:[UIApplication sharedApplication].keyWindow];
    if (!loading) {
        UIWindow *window = [UIApplication sharedApplication].keyWindow;
        SparkHudView *loading = [[SparkHudView alloc] initWithFrame:window.bounds];
